...
def get_ipo_data():
    ...
    existing_company_codes = [ipo["company_code"] for ipo in ipos]

    exclude_ipo_list_url = os.path.join(BASE_URL, "ipo/exceptions")
    ipos = requests.get(exclude_ipo_list_url).json()
    excluding_company_codes = [ipo["company_code"] for ipo in ipos]
    for company_code in excluding_company_codes:
        del ipo_data[company_code]
    ...
    for company_code, data in ipo_data.items():
        ...
        try:
            ...
            if company_code in existing_company_codes:
